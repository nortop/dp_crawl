...
// npm run crawl -- \                                            
//   --input candidate.csv \
//   --out observations_raw_dp3.csv \
//   --evidence evidence_dp3 \
//   --concurrency 2 \
//   --runs 2 \
//   --resume false \
//   --dp3 true \
//   --dp3WaitMs 6500 \
//   --dp3IncludeAccept true \
//   --dp3CaptureCanvasFp true
...
// IEEE S&P / Journal-grade DP3 (Pre-consent tracking) signatures.
// We intentionally focus on high-precision endpoint patterns to reduce false positives.
...
// ---------- Journal-grade DP3 differential measurement ----------

type ConsentState = "S0" | "S1" | "S2"; // S0=fresh, S1=reject, S2=accept
...
async function addCanvasFpInstrumentation(context: BrowserContext) {
  // Best-effort, low overhead. Flags canvas toDataURL usage as a proxy for fingerprinting attempts.
...
async function measureConsentState(
  ...
  // Pass interstitials (e.g., "เข้าเว็บไซต์") if your crawl.ts includes it.
  // If not present, this no-ops (we keep it guarded).
  try {
    // @ts-ignore
    if (typeof passInterstitials === "function") {
      // @ts-ignore
  ...
  if (state === "S1") {
    try {
      // Use existing reject logic if present; otherwise best-effort manage->confirm
      // @ts-ignore
...
async function runOne(browser: Browser, cand: CandidateRow, device: DeviceKind, runId: number): Promise<Obs> {
  ...
  const obs: Obs = {
    ...
    tracking_requests_count : "", 
    tracking_cookies_count : "",
  ...
  if (await detectTurnstile(page)) {
    ...
    // เพิ่มใน passInterstitials หลังจากคลิกปุ่มสำเร็จ
    await page.waitForFunction(() => {
      const body = document.body;
      const style = window.getComputedStyle(body);
      // ตรวจสอบ filter: grayscale(1) หรือ class ที่มีชื่อเกี่ยวกับ mourning
      const hasGrayscale = style.filter.includes('grayscale') || 
                          body.classList.contains('grayscale') ||
  ...
  if (rejectClicked) {
    ...
    try {
     ...
     // วัด DP5
      ...
      obs.tracking_after_reject_flag = tracking.trackingRequests > 0 ? 1 : 0; // ใช้ตัวแปรเดิม
...
const OUTPUT_HEADER = [
  ...
  "third_party_before_consent","third_party_after_reject","tracking_after_reject_flag","tracking_requests_count","tracking_cookies_count",
    ...
    limit(async () => {
      try {
        ...
        const fallback: Obs = {
          ...
          third_party_after_reject: "",
          tracking_after_reject_flag: "",
          tracking_requests_count : "", tracking_cookies_count : "",
          dp1_flag: "",
          dp2_flag: "",
          dp3_flag: "",
          dp4_flag: "",
...
async function passInterstitials(
  ...
  for (let step = 1; step <= maxSteps; step++) {
    ...
    // **เพิ่ม: หลังจากคลิกสำเร็จ ให้รอให้ grayscale หายไป** (14/02/2026)
    await page.waitForFunction(() => {
      const bodyStyle = window.getComputedStyle(document.body);
      // ตรวจสอบ filter: grayscale(1) หรือ class ที่เกี่ยวข้อง
      return !bodyStyle.filter.includes('grayscale') &&
             !document.body.classList.contains('grayscale') &&
             !document.documentElement.classList.contains('grayscale');
    }, { timeout: 10000 }).catch(() => {}); // ไม่ต้อง throw error ถ้า timeout
...
async function clickExitIntroFallback(page: Page): Promise<{ clicked: boolean; href: string }> {
  ...
  // หา <a> ที่ "อยู่โดเมนเดียวกัน" และ "ไม่ใช่ /intro" และมองเห็นได้
  ...
  for (let i = 0; i < Math.min(n, 60); i++) {
    ...
    if (p.includes("intro")) continue;              // หลีกเลี่ยงวน
    if (p === "/" || p.length > 1) {                // / หรือ path อื่นที่ไม่ใช่ intro
      try {
...
async function measureButtonSalience(page: Page): Promise<{
  ...
  try {
    ...
    // วัดสีพื้นหลัง (ถ้าไม่ใช่ grayscale)
    ...
    // เกณฑ์: areaRatio > 2 หรือ contrastDiff > 100 (สามารถปรับ threshold ได้)
    if (result.areaRatio > 2.0 || result.contrastDiff > 100) {
      result.dp4Flag = 1;
    }
  } catch (e) {}
...
// helper: ค้นหา element แรกที่ match regex
async function findButtonElement(page: Page, re: RegExp): Promise<Locator | null> {
  const loc = page.locator('button, a, [role="button"], input[type="button"], input[type="submit"]').filter({ hasText: re });
  const count = await loc.count();
  for (let i = 0; i < count; i++) {
    if (await loc.nth(i).isVisible()) return loc.nth(i);
...
async function detectCookieWall(page: Page): Promise<number> {
  try {
    // ดูว่า main content ซ่อนอยู่หรือไม่
    const main = page.locator('main, article, #content, .content, body').first();
    const visible = await main.isVisible().catch(() => true);
    if (!visible) return 1;

    // ตรวจสอบ overlay ที่อาจบัง
...
async function measureTrackingAfterReject(page: Page, context: BrowserContext): Promise<{
  ...
  await page.waitForTimeout(3000); // รอให้ requests เกิดขึ้น

  page.off('request', handler);

  // นับ tracking requests
  const trackingReqs = requests.filter(url =>
    TRACKING_ENDPOINT_PATTERNS.some(re => re.test(url))
  ).length;

  // นับ tracking cookies
...
async function detectWithdrawalDifficulty(page: Page): Promise<number> {
  try {
    const links = page.locator(
      'a:has-text("จัดการคุกกี้"), a:has-text("Cookie settings"), a:has-text("Privacy settings"), a[href*="cookie"], a[href*="privacy"], footer a'
    );
    const count = await links.count();
    for (let i = 0; i < count; i++) {
      if (await links.nth(i).isVisible()) return 0; // มีลิงก์ที่มองเห็น
    }
    return 1; // ไม่พบ
  } catch {
    return 1;
