import fs from "node:fs";
import path from "node:path";
import { parse as parseCsv } from "csv-parse/sync";
import pLimit from "p-limit";
import { chromium, Browser, BrowserContext, Locator, Page, Request } from "playwright";
import { getDomain } from "tldts";

// ---------- CLI args ----------
type Args = {
  input: string;
  out: string;
  evidence: string;
  concurrency: number;
  runs: number;
  headful: boolean;
  timeoutMs: number;
  settleMs: number;
  locale: string;
  resume: boolean;
  dp3: boolean;
  dp3WaitMs: number;
  dp3IncludeAccept: boolean;
  dp3CaptureCanvasFp: boolean;
};

function parseArgs(argv: string[]): Args {
  const a: any = {};
  for (let i = 0; i < argv.length; i++) {
    const k = argv[i];
    if (!k.startsWith("--")) continue;
    const key = k.slice(2);
    const next = argv[i + 1];
    if (!next || next.startsWith("--")) {
      a[key] = true;
    } else {
      a[key] = next;
      i++;
    }
  }
  return {
    input: String(a.input ?? "candidate.csv"),
    out: String(a.out ?? "observations_raw.csv"),
    evidence: String(a.evidence ?? "evidence"),
    concurrency: Number(a.concurrency ?? 3),
    runs: Number(a.runs ?? 2),
    headful: Boolean(a.headful ?? false),
    timeoutMs: Number(a.timeoutMs ?? 30000),
    settleMs: Number(a.settleMs ?? 3500),
    locale: String(a.locale ?? "th-TH"),
    resume: Boolean(a.resume ?? true),
    dp3: Boolean(a.dp3 ?? true),
    dp3WaitMs: Number(a.dp3WaitMs ?? 5000),
    dp3IncludeAccept: Boolean(a.dp3IncludeAccept ?? true),
    dp3CaptureCanvasFp: Boolean(a.dp3CaptureCanvasFp ?? true)
  };
}

const ARGS = parseArgs(process.argv.slice(2));

// ---------- Patterns ----------
const TRACKING_ENDPOINT_PATTERNS: RegExp[] = [
  /google-analytics\.com\/(g\/collect|collect)/i,
  /google-analytics\.com\/(analytics\.js|ga\.js)/i,
  /googletagmanager\.com\/gtm\.js/i,
  /doubleclick\.net\/.+/i,
  /googleads\.g\.doubleclick\.net\/.+/i,
  /googlesyndication\.com\/.+/i,
  /facebook\.com\/tr\?/i,
  /connect\.facebook\.net\/.+\/fbevents\.js/i,
  /static\.hotjar\.com\/.+/i,
  /analytics\.tiktok\.com\/.+/i
];

const TRACKING_COOKIE_NAME_PATTERNS: RegExp[] = [
  /^_ga/i, /^_gid/i, /^_gat/i,
  /^_gcl_/i,
  /^_fbp/i, /^_fbc/i,
  /^_hj/i,
];

const ACCEPT_RE = /(accept all|allow all|agree|i accept|ok|yes|ยอมรับทั้งหมด|ยอมรับ|ตกลง|อนุญาตทั้งหมด|ยินยอม)/i;
const REJECT_RE = /(reject all|decline all|deny all|do not accept|no thanks|ปฏิเสธทั้งหมด|ปฏิเสธ|ไม่ยอมรับ|ไม่อนุญาต|ไม่ยินยอม)/i;
const REJECT_ALL_STRICT_RE = /(reject all|decline all|deny all|ปฏิเสธทั้งหมด)/i;
const MANAGE_RE = /(manage|settings|preferences|options|customize|more info|ตั้งค่า|จัดการ|ตัวเลือก|การตั้งค่า|ปรับแต่ง|รายละเอียดเพิ่มเติม)/i;
const CLOSE_RE = /(\bclose\b|dismiss|x|ปิด|ไม่ตอนนี้|ภายหลัง)/i;

const COOKIE_KEYWORDS_RE = /(cookie|cookies|consent|privacy|tracking|คุกกี้|ความเป็นส่วนตัว|ความยินยอม|ติดตาม|โฆษณา)/i;

const TOGGLE_ANALYTICS_RE = /(analytics|statistics|measurement|performance|วิเคราะห์|สถิติ|ประสิทธิภาพ)/i;
const TOGGLE_ADS_RE = /(ads|advertis|marketing|target|personaliz|remarket|การตลาด|โฆษณา|กำหนดเป้าหมาย|ปรับแต่ง)/i;

const ENTER_SITE_RE = /(เข้าเว็บไซต์|เข้าสู่เว็บไซต์|ไปยังเว็บไซต์|เข้าหน้าเว็บไซต์|ไปยังหน้าเว็บไซต์|เริ่มใช้งาน|ดำเนินการต่อ|ต่อไป|ยืนยันและไปต่อ|Enter (the )?site|Continue( to)?( site)?|Proceed|Go to site|I understand|Agree and continue)/i;

const CONFIRM_PREFS_RE = /(ยืนยันตัวเลือกของฉัน|ยืนยันตัวเลือก|บันทึก(การตั้งค่า)?|บันทึกและปิด|ตกลง|ยืนยัน|Save( settings)?|Confirm( choices)?|Accept selected|Allow selected)/i;

const ACCEPT_ALL_RE = /(ยอมรับทั้งหมด|accept all|allow all|agree|ตกลง|ok)/i;

const COOKIE_WORD_RE = /(cookie|cookies|คุกกี้)/i;

const BLOCKED_PAGE_KEYWORDS = [
  "blocked", "restricted", "access denied", "forbidden", "cannot be reached", "ตรวจสอบความปลอดภัย",
  "site cannot be reached", "this site can’t be reached", "拒绝访问", "被阻止"
];

// ---------- Types ----------
type CandidateRow = {
  domain: string;
  stratum: string;
  source?: string;
  source_rank?: string;
};

type DeviceKind = "desktop" | "mobile";

type Obs = Record<string, any>;

// ---------- Helpers ----------
function ensureDir(p: string) {
  fs.mkdirSync(p, { recursive: true });
}

function nowIsoUtc() {
  return new Date().toISOString();
}

function safeSlug(s: string) {
  return s.replace(/[^a-zA-Z0-9._-]+/g, "_");
}

function normalizeDomain(raw: unknown): string {
  if (typeof raw !== "string") return "";
  let x = raw.trim();
  x = x.replace(/^https?:\/\//, "");
  x = x.split("/")[0];
  if (x.startsWith("www.")) x = x.slice(4);
  return x.toLowerCase();
}

function domainRoot(host: string): string {
  const d = getDomain(host);
  return d || host;
}

function hostOf(url: string): string {
  try {
    return new URL(url).hostname.toLowerCase();
  } catch {
    return "";
  }
}

function isThirdParty(reqHost: string, siteHost: string): boolean {
  if (!reqHost || !siteHost) return false;
  const r1 = domainRoot(reqHost);
  const r2 = domainRoot(siteHost);
  return r1 !== r2;
}

function writeCsvRowAppend(outPath: string, header: string[], row: Record<string, any>) {
  const exists = fs.existsSync(outPath);
  const line = header
    .map((h) => {
      const v = row[h];
      const s = v === undefined || v === null ? "" : String(v);
      const esc = s.includes(",") || s.includes('"') || s.includes("\n");
      return esc ? `"${s.replace(/"/g, '""')}"` : s;
    })
    .join(",") + "\n";

  if (!exists) {
    fs.writeFileSync(outPath, header.join(",") + "\n", "utf-8");
  }
  fs.appendFileSync(outPath, line, "utf-8");
}

function loadExistingKeys(outPath: string): Set<string> {
  const keys = new Set<string>();
  if (!fs.existsSync(outPath)) return keys;
  const txt = fs.readFileSync(outPath, "utf-8");
  const lines = txt.split("\n").filter(Boolean);
  if (lines.length < 2) return keys;
  const header = lines[0].split(",");
  const idxDomain = header.indexOf("domain");
  const idxDevice = header.indexOf("device");
  const idxRun = header.indexOf("run_id");
  if (idxDomain < 0 || idxDevice < 0 || idxRun < 0) return keys;

  for (let i = 1; i < lines.length; i++) {
    const cols = parseCsvLine(lines[i]);
    const dom = cols[idxDomain] ?? "";
    const dev = cols[idxDevice] ?? "";
    const run = cols[idxRun] ?? "";
    if (dom && dev && run) keys.add(`${dom}|${dev}|${run}`);
  }
  return keys;
}

function parseCsvLine(line: string): string[] {
  const out: string[] = [];
  let cur = "";
  let inQ = false;
  for (let i = 0; i < line.length; i++) {
    const ch = line[i];
    if (inQ) {
      if (ch === '"') {
        if (line[i + 1] === '"') {
          cur += '"';
          i++;
        } else {
          inQ = false;
        }
      } else {
        cur += ch;
      }
    } else {
      if (ch === ",") {
        out.push(cur);
        cur = "";
      } else if (ch === '"') {
        inQ = true;
      } else {
        cur += ch;
      }
    }
  }
  out.push(cur);
  return out;
}

function progressTag(domain: string, device: DeviceKind, runId: number): string {
  return `[${domain}|${device}|run${runId}]`;
}

function logStep(domain: string, device: DeviceKind, runId: number, step: string) {
  console.log(`${progressTag(domain, device, runId)} ${step}`);
}

// ---------- Page scanning ----------
async function detectLang(page: Page): Promise<string> {
  try {
    return await page.evaluate(() => document.documentElement.lang || "");
  } catch {
    return "";
  }
}

async function detectCmpVendor(page: Page): Promise<string> {
  try {
    const scripts: string[] = await page.evaluate(() =>
      Array.from(document.scripts)
        .map((s) => s.src || "")
        .filter(Boolean)
    );
    const s = scripts.join("\n").toLowerCase();
    const vendors: [string, RegExp][] = [
      ["OneTrust", /onetrust|cookielaw/i],
      ["Cookiebot", /cookiebot/i],
      ["TrustArc", /trustarc|truste/i],
      ["Didomi", /didomi/i],
      ["Quantcast", /quantcast/i],
      ["Osano", /osano/i],
      ["Sourcepoint", /sourcepoint/i],
      ["Iubenda", /iubenda/i],
      ["Complianz", /complianz/i],
    ];
    for (const [name, re] of vendors) {
      if (re.test(s)) return name;
    }
    return "unknown";
  } catch {
    return "unknown";
  }
}

async function bannerPresentHeuristic(page: Page): Promise<number> {
  try {
    const txt: string = await page.evaluate(() => (document.body?.innerText || "").slice(0, 20000));
    return COOKIE_KEYWORDS_RE.test(txt) ? 1 : 0;
  } catch {
    return 0;
  }
}

async function findClickableByText(page: Page, re: RegExp): Promise<{ text: string } | null> {
  const loc = page
    .locator("button, a, [role='button'], input[type='button'], input[type='submit'], div[role='button']")
    .filter({ hasText: re });

  const n = await loc.count().catch(() => 0);
  if (n === 0) return null;

  for (let i = 0; i < Math.min(n, 20); i++) {
    const el = loc.nth(i);
    const vis = await el.isVisible().catch(() => false);
    if (!vis) continue;

    const text =
      (await el.innerText().catch(() => "")) ||
      (await el.getAttribute("value").catch(() => "")) ||
      "";
    return { text: text.trim() };
  }
  return null;
}

async function clickBestEffort(page: Page, re: RegExp): Promise<boolean> {
  try {
    const loc = page
      .locator("button, a, [role='button'], input[type='button'], input[type='submit'], div[role='button']")
      .filter({ hasText: re });

    if (await loc.first().isVisible({ timeout: 1200 }).catch(() => false)) {
      await loc.first().click({ timeout: 2000, force: true });
      return true;
    }
  } catch {}
  return false;
}

async function clickByRegex(
  page: Page,
  include: RegExp,
  opts?: { exclude?: RegExp; maxScan?: number; force?: boolean }
): Promise<{ clicked: boolean; text: string }> {
  const exclude = opts?.exclude;
  const maxScan = opts?.maxScan ?? 30;

  const loc = page.locator(
    "button, a, [role='button'], input[type='button'], input[type='submit'], div[role='button']"
  );

  const n = await loc.count().catch(() => 0);
  for (let i = 0; i < Math.min(n, maxScan); i++) {
    const el = loc.nth(i);
    const vis = await el.isVisible().catch(() => false);
    if (!vis) continue;

    const txt =
      (await el.innerText().catch(() => "")) ||
      (await el.getAttribute("value").catch(() => "")) ||
      "";
    const t = txt.trim();

    if (!t) continue;
    if (!include.test(t)) continue;
    if (exclude && exclude.test(t)) continue;

    try {
      await el.click({ timeout: 2000, force: opts?.force ?? true });
      return { clicked: true, text: t };
    } catch {
      // continue scan
    }
  }

  return { clicked: false, text: "" };
}

async function tickVisibleCheckboxesBestEffort(page: Page, maxTicks = 3): Promise<number> {
  let ticks = 0;

  const cb = page.locator("input[type='checkbox']");
  const n1 = await cb.count().catch(() => 0);
  for (let i = 0; i < Math.min(n1, 20) && ticks < maxTicks; i++) {
    const el = cb.nth(i);
    const vis = await el.isVisible().catch(() => false);
    if (!vis) continue;

    const checked = await el.isChecked().catch(() => false);
    if (!checked) {
      try {
        await el.check({ timeout: 1500, force: true });
        ticks++;
      } catch {
        try {
          await el.click({ timeout: 1500, force: true });
          ticks++;
        } catch {}
      }
    }
  }

  const sw = page.locator("[role='checkbox'], [role='switch']");
  const n2 = await sw.count().catch(() => 0);
  for (let i = 0; i < Math.min(n2, 20) && ticks < maxTicks; i++) {
    const el = sw.nth(i);
    const vis = await el.isVisible().catch(() => false);
    if (!vis) continue;

    const aria = (await el.getAttribute("aria-checked").catch(() => "")) || "";
    if (aria === "false") {
      try {
        await el.click({ timeout: 1500, force: true });
        ticks++;
      } catch {}
    }
  }

  return ticks;
}

async function getToggleLabel(el: Locator): Promise<string> {
  return await el
    .evaluate((node) => {
      const e = node as HTMLElement;
      const aria = e.getAttribute("aria-label") || "";
      const id = (e as any).id || "";
      let lbl = "";

      if (id) {
        const lab = document.querySelector(`label[for="${CSS.escape(id)}"]`);
        if (lab) lbl = (lab.textContent || "").trim();
      }

      if (!lbl) {
        const closestLabel = e.closest("label");
        if (closestLabel) lbl = (closestLabel.textContent || "").trim();
      }

      const parentText = (e.parentElement?.textContent || "").trim();
      return (aria || lbl || parentText).slice(0, 200);
    })
    .catch(() => "");
}

async function scanTogglesBestEffort(page: Page): Promise<{
  has_analytics: number;
  has_ads: number;
  default_on_analytics: number;
  default_on_ads: number;
}> {
  try {
    const res = await page.evaluate(() => {
      const items: { label: string; checked: boolean }[] = [];

      function labelText(el: Element): string {
        const aria = (el as HTMLElement).getAttribute("aria-label") || "";
        const id = (el as HTMLElement).id;
        let lbl = "";
        if (id) {
          const lab = document.querySelector(`label[for="${CSS.escape(id)}"]`);
          if (lab) lbl = (lab.textContent || "").trim();
        }
        const closestLabel = el.closest("label");
        if (!lbl && closestLabel) lbl = (closestLabel.textContent || "").trim();
        const parentText = (el.parentElement?.textContent || "").trim();
        return (aria || lbl || parentText).slice(0, 200);
      }

      document.querySelectorAll("input[type='checkbox']").forEach((el) => {
        const input = el as HTMLInputElement;
        items.push({ label: labelText(input), checked: input.checked });
      });

      document.querySelectorAll("[role='switch']").forEach((el) => {
        const v = (el as HTMLElement).getAttribute("aria-checked");
        const checked = v === "true";
        items.push({ label: labelText(el), checked });
      });

      return items;
    });

    let hasAnalytics = 0,
      hasAds = 0,
      onAnalytics = 0,
      onAds = 0;

    for (const it of res) {
      const label = (it.label || "").toLowerCase();
      if (TOGGLE_ANALYTICS_RE.test(label)) {
        hasAnalytics = 1;
        if (it.checked) onAnalytics = 1;
      }
      if (TOGGLE_ADS_RE.test(label)) {
        hasAds = 1;
        if (it.checked) onAds = 1;
      }
    }
    return {
      has_analytics: hasAnalytics,
      has_ads: hasAds,
      default_on_analytics: onAnalytics,
      default_on_ads: onAds,
    };
  } catch {
    return { has_analytics: 0, has_ads: 0, default_on_analytics: 0, default_on_ads: 0 };
  }
}

async function toggleOffNonEssentialBestEffort(page: Page): Promise<{
  has_analytics: number;
  has_ads: number;
  default_on_analytics: number;
  default_on_ads: number;
  toggles_clicked: number;
}> {
  let hasAnalytics = 0, hasAds = 0;
  let onAnalytics = 0, onAds = 0;
  let togglesClicked = 0;

  const cbs = page.locator("input[type='checkbox']");
  const nCb = await cbs.count().catch(() => 0);
  for (let i = 0; i < Math.min(nCb, 30); i++) {
    const el = cbs.nth(i);
    if (!(await el.isVisible().catch(() => false))) continue;

    const label = (await getToggleLabel(el)).toLowerCase();
    const isAnalytics = TOGGLE_ANALYTICS_RE.test(label);
    const isAds = TOGGLE_ADS_RE.test(label);
    if (!isAnalytics && !isAds) continue;

    const checked = await el.isChecked().catch(() => false);
    if (isAnalytics) { hasAnalytics = 1; if (checked) onAnalytics = 1; }
    if (isAds) { hasAds = 1; if (checked) onAds = 1; }

    if (checked) {
      await el.click({ timeout: 1500, force: true }).catch(() => {});
      togglesClicked++;
    }
  }

  const sw = page.locator("[role='switch'], [role='checkbox']");
  const nSw = await sw.count().catch(() => 0);
  for (let i = 0; i < Math.min(nSw, 30); i++) {
    const el = sw.nth(i);
    if (!(await el.isVisible().catch(() => false))) continue;

    const label = (await getToggleLabel(el)).toLowerCase();
    const isAnalytics = TOGGLE_ANALYTICS_RE.test(label);
    const isAds = TOGGLE_ADS_RE.test(label);
    if (!isAnalytics && !isAds) continue;

    const aria = (await el.getAttribute("aria-checked").catch(() => "")) || "";
    const checked = aria === "true";

    if (isAnalytics) { hasAnalytics = 1; if (checked) onAnalytics = 1; }
    if (isAds) { hasAds = 1; if (checked) onAds = 1; }

    if (checked) {
      await el.click({ timeout: 1500, force: true }).catch(() => {});
      togglesClicked++;
    }
  }

  return {
    has_analytics: hasAnalytics,
    has_ads: hasAds,
    default_on_analytics: onAnalytics,
    default_on_ads: onAds,
    toggles_clicked: togglesClicked,
  };
}

async function detectTurnstile(page: Page): Promise<boolean> {
  try {
    const hasIframe = await page
      .locator("iframe[src*='challenges.cloudflare.com']")
      .count();

    const bodyText = await page.locator("body").innerText().catch(() => "");
    const textFlag = /cloudflare|verify you are human|ตรวจสอบว่าคุณเป็นมนุษย์/i.test(bodyText);

    return hasIframe > 0 || textFlag;
  } catch {
    return false;
  }
}

async function isBlockedPage(page: Page): Promise<boolean> {
  try {
    const title = await page.title().catch(() => "");
    const body = await page.locator("body").innerText().catch(() => "");
    const content = (title + " " + body).toLowerCase();
    for (const kw of BLOCKED_PAGE_KEYWORDS) {
      if (content.includes(kw.toLowerCase())) return true;
    }
    return false;
  } catch {
    return false;
  }
}

async function clickExitIntroFallback(page: Page): Promise<{ clicked: boolean; href: string }> {
  const current = page.url();
  let host = "";
  try { host = new URL(current).hostname.replace(/^www\./, ""); } catch {}

  const links = page.locator("a[href]");
  const n = await links.count().catch(() => 0);

  for (let i = 0; i < Math.min(n, 60); i++) {
    const a = links.nth(i);
    if (!(await a.isVisible().catch(() => false))) continue;

    const href = (await a.getAttribute("href").catch(() => "")) || "";
    if (!href) continue;

    let u: URL | null = null;
    try { u = new URL(href, current); } catch { u = null; }
    if (!u) continue;

    const h = u.hostname.replace(/^www\./, "");
    if (host && h !== host) continue;

    const p = (u.pathname || "").toLowerCase();
    if (p.includes("intro")) continue;
    if (p === "/" || p.length > 1) {
      try {
        await a.click({ timeout: 2500, force: true });
        return { clicked: true, href: u.toString() };
      } catch {}
    }
  }

  return { clicked: false, href: "" };
}

async function passInterstitials(
  page: Page,
  obs: Record<string, any>,
  evidenceDir: string,
  runId: number,
  maxSteps = 3
): Promise<void> {
  for (let step = 1; step <= maxSteps; step++) {
    const shotBefore = path.join(evidenceDir, `run${runId}_interstitial_step${step}_before.png`);
    await page.screenshot({ path: shotBefore }).catch(() => {});
    obs.notes = (obs.notes || "") + `interstitial_step${step}_before=${shotBefore}; `;

    const click1 = await clickByRegex(page, ENTER_SITE_RE, { exclude: COOKIE_WORD_RE, maxScan: 40, force: true });

    if (!click1.clicked) {
      const fb = await clickExitIntroFallback(page);
      if (!fb.clicked) break;
      obs.notes = (obs.notes || "") + `interstitial_fallback_clicked_href=${fb.href}; `;
      await Promise.race([
        page.waitForNavigation({ timeout: 5000, waitUntil: "domcontentloaded" }).catch(() => null),
        page.waitForLoadState("domcontentloaded", { timeout: 5000 }).catch(() => null),
        page.waitForTimeout(2000),
      ]);
    }

    obs.notes = (obs.notes || "") + `interstitial_clicked="${click1.text}"; `;

    await Promise.race([
      page.waitForNavigation({ timeout: 5000, waitUntil: "domcontentloaded" }).catch(() => null),
      page.waitForLoadState("domcontentloaded", { timeout: 5000 }).catch(() => null),
      page.waitForTimeout(2000),
    ]);

    // Wait for grayscale to disappear (mourning page)
    await page.waitForFunction(() => {
      const bodyStyle = window.getComputedStyle(document.body);
      return !bodyStyle.filter.includes('grayscale') &&
             !document.body.classList.contains('grayscale') &&
             !document.documentElement.classList.contains('grayscale');
    }, { timeout: 5000 }).catch(() => {});

    const urlAfter = page.url();
    const shotAfter = path.join(evidenceDir, `run${runId}_interstitial_step${step}_after.png`);
    await page.screenshot({ path: shotAfter }).catch(() => {});
    obs.notes = (obs.notes || "") + `interstitial_step${step}_after=${shotAfter}; `;

    const stillInterstitial = await page
      .locator("body")
      .innerText()
      .then((t) => ENTER_SITE_RE.test((t || "").slice(0, 20000)))
      .catch(() => false);

    if (stillInterstitial) {
      const ticks = await tickVisibleCheckboxesBestEffort(page, 2);
      if (ticks > 0) {
        obs.notes = (obs.notes || "") + `interstitial_checkboxes_ticked=${ticks}; `;
        const click2 = await clickByRegex(page, ENTER_SITE_RE, { exclude: COOKIE_WORD_RE, maxScan: 40, force: true });
        if (click2.clicked) {
          await Promise.race([
            page.waitForNavigation({ timeout: 5000, waitUntil: "domcontentloaded" }).catch(() => null),
            page.waitForLoadState("domcontentloaded", { timeout: 5000 }).catch(() => null),
            page.waitForTimeout(2000),
          ]);
        }
      }
    }

    const still2 = await page
      .locator("body")
      .innerText()
      .then((t) => ENTER_SITE_RE.test((t || "").slice(0, 20000)))
      .catch(() => false);

    if (!still2) break;
  }
}

// ---------- DP measurement functions ----------

// DP4: Visual Salience
async function measureButtonSalience(page: Page): Promise<{
  acceptArea: number;
  rejectArea: number;
  areaRatio: number;
  contrastDiff: number;
  dp4Flag: number;
}> {
  const result = { acceptArea: 0, rejectArea: 0, areaRatio: 1, contrastDiff: 0, dp4Flag: 0 };

  const findBtn = async (re: RegExp) => {
    const loc = page.locator('button, a, [role="button"], input[type="button"], input[type="submit"]').filter({ hasText: re });
    const count = await loc.count();
    for (let i = 0; i < count; i++) {
      if (await loc.nth(i).isVisible()) return loc.nth(i);
    }
    return null;
  };

  try {
    const acceptBtn = await findBtn(ACCEPT_RE);
    const rejectBtn = await findBtn(REJECT_RE);
    if (!acceptBtn || !rejectBtn) return result;

    const acceptBox = await acceptBtn.boundingBox();
    const rejectBox = await rejectBtn.boundingBox();
    if (!acceptBox || !rejectBox) return result;

    result.acceptArea = acceptBox.width * acceptBox.height;
    result.rejectArea = rejectBox.width * rejectBox.height;
    result.areaRatio = result.acceptArea / (result.rejectArea || 1);

    const isGrayscale = await page.evaluate(() =>
      window.getComputedStyle(document.body).filter.includes('grayscale')
    ).catch(() => false);

    if (!isGrayscale) {
      const acceptColor = await acceptBtn.evaluate(el => window.getComputedStyle(el).backgroundColor);
      const rejectColor = await rejectBtn.evaluate(el => window.getComputedStyle(el).backgroundColor);
      const luminance = (color: string) => {
        const rgb = color.match(/\d+/g)?.map(Number) || [0, 0, 0];
        return 0.2126 * rgb[0] + 0.7152 * rgb[1] + 0.0722 * rgb[2];
      };
      result.contrastDiff = Math.abs(luminance(acceptColor) - luminance(rejectColor));
    }

    // Adjusted thresholds
    if (result.areaRatio > 1.5 || result.contrastDiff > 50) {
      result.dp4Flag = 1;
    }
  } catch (e) {}

  return result;
}

// DP5: Cookie Wall
async function detectCookieWall(page: Page): Promise<number> {
  try {
    const mainSelectors = ['main', 'article', '#content', '.content', 'body'];
    for (const sel of mainSelectors) {
      const main = page.locator(sel).first();
      if (await main.count() > 0) {
        const visible = await main.isVisible().catch(() => true);
        if (!visible) return 1;
      }
    }

    const overlay = page.locator('div[class*="overlay"], div[class*="modal"], div[style*="fixed"]').first();
    if (await overlay.isVisible()) {
      const text = await overlay.innerText().catch(() => '');
      if (/cookie|cookies|คุกกี้/i.test(text)) return 1;
    }
    return 0;
  } catch {
    return 0;
  }
}

// DP6: Vagueness
function detectVagueness(bannerText: string): number {
  const lower = bannerText.toLowerCase();
  const hasCategories = /(analytics|statistics|marketing|advertising|personalization|necessary|functional|required|วิเคราะห์|สถิติ|การตลาด|โฆษณา|ปรับแต่ง|จำเป็น|การทำงาน)/i.test(lower);
  const vaguePhrases = ['เพื่อประสบการณ์ที่ดีขึ้น', 'improve experience', 'enhance service', 'better browsing', 'เพื่อวัตถุประสงค์อื่น', 'for other purposes'];
  const hasVague = vaguePhrases.some(p => lower.includes(p));

  return (hasVague && !hasCategories) ? 1 : 0;
}

// DP7: Choice Not Respected (improved)
async function measureTrackingAfterReject(page: Page, context: BrowserContext): Promise<{
  trackingRequests: number;
  trackingCookies: number;
  dp7Flag: number;
}> {
  const requests: string[] = [];
  const handler = (req: any) => {
    try { requests.push(req.url()); } catch {}
  };
  page.on('request', handler);
  await page.waitForTimeout(5000); // wait for requests
  page.off('request', handler);

  const trackingReqs = requests.filter(url =>
    TRACKING_ENDPOINT_PATTERNS.some(re => re.test(url))
  ).length;

  const cookies = await context.cookies();
  const trackingCookies = cookies.filter(c =>
    TRACKING_COOKIE_NAME_PATTERNS.some(re => re.test(c.name))
  ).length;

  const dp7Flag = (trackingReqs > 0 || trackingCookies > 0) ? 1 : 0;
  return { trackingRequests: trackingReqs, trackingCookies, dp7Flag };
}

// DP8: Withdrawal Difficulty (improved)
async function detectWithdrawalDifficulty(page: Page): Promise<number> {
  try {
    const links = page.locator(
      'a:has-text("จัดการคุกกี้"), a:has-text("Cookie settings"), a:has-text("Privacy settings"), a:has-text("Cookie preferences"), ' +
      'a:has-text("ตั้งค่าความเป็นส่วนตัว"), a[href*="cookie"], a[href*="privacy"], footer a'
    );
    const count = await links.count();
    for (let i = 0; i < count; i++) {
      const link = links.nth(i);
      if (await link.isVisible()) {
        const href = await link.getAttribute('href');
        if (href && !href.startsWith('#') && !href.startsWith('javascript')) {
          return 0; // found a manageable link
        }
      }
    }
    return 1;
  } catch {
    return 1;
  }
}

// Helper to accept via preferences (for DP2 completeness)
async function acceptViaPreferences(page: Page, obs: Record<string, any>, evidenceDir: string, runId: number): Promise<{ ok: boolean; clickCount: number; details: string }> {
  const opened = await clickBestEffort(page, MANAGE_RE);
  if (!opened) return { ok: false, clickCount: 0, details: "manage_not_found" };
  await page.waitForTimeout(1500);

  const acceptAllInside = await findClickableByText(page, ACCEPT_ALL_RE);
  if (!acceptAllInside) {
    const clicked = await clickByRegex(page, CONFIRM_PREFS_RE, { exclude: REJECT_RE });
    if (!clicked.clicked) return { ok: false, clickCount: 1, details: "no_accept_found" };
    return { ok: true, clickCount: 2, details: "manage+confirm" };
  }

  await clickBestEffort(page, ACCEPT_ALL_RE);
  return { ok: true, clickCount: 2, details: "manage+accept_all" };
}

// Enhanced rejectViaPreferences
async function rejectViaPreferences(
  page: Page,
  obs: Record<string, any>,
  evidenceDir: string,
  runId: number,
  onSetupReady?: () => Promise<void> | void
): Promise<{ ok: boolean; clickCount: number; details: string }> {
  let clickCount = 0;

  const opened = await clickBestEffort(page, MANAGE_RE);
  if (!opened) return { ok: false, clickCount, details: "manage_not_found" };
  clickCount++;
  await page.waitForTimeout(1500);

  const rejectAllInside = await findClickableByText(page, REJECT_ALL_STRICT_RE);
  if (rejectAllInside) {
    const manageShot = path.join(evidenceDir, `run${runId}_manage.png`);
    await page.screenshot({ path: manageShot }).catch(() => {});
    obs.screenshot_path_manage = manageShot;
    await onSetupReady?.();
    await clickBestEffort(page, REJECT_ALL_STRICT_RE);
    clickCount++;
    const confirm = await clickByRegex(page, CONFIRM_PREFS_RE, { exclude: ACCEPT_ALL_RE });
    if (confirm.clicked) clickCount++;
    return { ok: true, clickCount, details: "manage+reject_all_inside" };
  }

  const t = await toggleOffNonEssentialBestEffort(page);
  clickCount += t.toggles_clicked;

  const manageShot = path.join(evidenceDir, `run${runId}_manage.png`);
  await page.screenshot({ path: manageShot }).catch(() => {});
  obs.screenshot_path_manage = manageShot;
  await onSetupReady?.();

  const clicked = await clickByRegex(page, CONFIRM_PREFS_RE, { exclude: ACCEPT_ALL_RE, maxScan: 40 });
  if (clicked.clicked) {
    clickCount++;
    return { ok: true, clickCount, details: `manage+${t.toggles_clicked}toggles+confirm` };
  }

  return { ok: false, clickCount, details: `manage+${t.toggles_clicked}toggles+confirm_failed` };
}

// ---------- Consent State Recorder (for DP3 differential) ----------
type ConsentState = "S0" | "S1" | "S2";

type ConsentStateSummary = {
  final_url: string;
  endpoints_all: Set<string>;
  endpoints_tracking: Set<string>;
  set_cookie_headers: Set<string>;
  cookies: any[];
  tracking_cookies: any[];
  local_storage_keys: string[];
  session_storage_keys: string[];
  canvas_fp_called: boolean;
};

class ConsentStateRecorder {
  private reqUrls: string[] = [];
  private setCookieHeaders: string[] = [];
  private canvasFpCalled: boolean = false;

  attach(page: Page) {
    page.on("request", (req) => {
      try { this.reqUrls.push(req.url()); } catch {}
    });
    page.on("response", async (res) => {
      try {
        const h = res.headers();
        const sc = h["set-cookie"];
        if (sc) this.setCookieHeaders.push(sc);
      } catch {}
    });
  }

  async snapshot(page: Page, context: BrowserContext, captureCanvasFp: boolean): Promise<Omit<ConsentStateSummary, "final_url" | "endpoints_all" | "endpoints_tracking" | "set_cookie_headers">> {
    const cookies = await context.cookies().catch(() => []);
    const trackingCookies = (cookies || []).filter((c: any) =>
      TRACKING_COOKIE_NAME_PATTERNS.some((re) => re.test(String(c?.name || "")))
    );

    const stor = await page.evaluate(() => {
      const lk = Object.keys(localStorage || {});
      const sk = Object.keys(sessionStorage || {});
      const canvas = (window as any).__dp_canvas_fp_called === true;
      return { lk, sk, canvas };
    }).catch(() => ({ lk: [], sk: [], canvas: false }));

    if (captureCanvasFp) this.canvasFpCalled = Boolean((stor as any).canvas);

    return {
      cookies,
      tracking_cookies: trackingCookies,
      local_storage_keys: (stor as any).lk || [],
      session_storage_keys: (stor as any).sk || [],
      canvas_fp_called: this.canvasFpCalled,
    };
  }

  summarizeTracking(): { endpoints_all: Set<string>; endpoints_tracking: Set<string>; set_cookie_headers: Set<string> } {
    const endpointsAll = new Set<string>(this.reqUrls);
    const endpointsTracking = new Set<string>();
    for (const u of endpointsAll) {
      for (const re of TRACKING_ENDPOINT_PATTERNS) {
        if (re.test(u)) { endpointsTracking.add(u); break; }
      }
    }
    return { endpoints_all: endpointsAll, endpoints_tracking: endpointsTracking, set_cookie_headers: new Set(this.setCookieHeaders) };
  }
}

function differentialSet(a: Set<string>, b: Set<string>): string[] {
  const out: string[] = [];
  for (const x of a) if (!b.has(x)) out.push(x);
  return out;
}

function differentialCookies(a: any[], b: any[]): any[] {
  const bNames = new Set((b || []).map((c: any) => String(c?.name || "")));
  return (a || []).filter((c: any) => !bNames.has(String(c?.name || "")));
}

async function addCanvasFpInstrumentation(context: BrowserContext) {
  await context.addInitScript(() => {
    try {
      (window as any).__dp_canvas_fp_called = false;
      const orig = HTMLCanvasElement.prototype.toDataURL;
      HTMLCanvasElement.prototype.toDataURL = function (...args: any[]) {
        (window as any).__dp_canvas_fp_called = true;
        // @ts-ignore
        return orig.apply(this, args);
      };
    } catch {}
  });
}

async function measureConsentState(
  browser: Browser,
  cand: CandidateRow,
  device: DeviceKind,
  state: ConsentState,
  waitMs: number,
  captureCanvasFp: boolean
): Promise<ConsentStateSummary> {

  const domain = normalizeDomain(cand.domain);
  const url = domain.startsWith("http") ? domain : `https://${domain}`;

  const contextOptions: any = {
    locale: ARGS.locale,
    ignoreHTTPSErrors: true,
    viewport: device === "mobile" ? { width: 390, height: 844 } : { width: 1366, height: 768 },
    userAgent:
      device === "mobile"
        ? "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Mobile/15E148 Safari/604.1"
        : undefined,
    isMobile: device === "mobile",
  };

  const context = await browser.newContext(contextOptions);
  if (captureCanvasFp) await addCanvasFpInstrumentation(context).catch(() => {});

  const page = await context.newPage();
  const rec = new ConsentStateRecorder();
  rec.attach(page);

  // Navigate (https then http fallback) with minimal duplication of main runner
  let navigated = false;
  let finalUrl = "";
  const tryUrls = cand.domain.startsWith("http")
    ? [cand.domain]
    : [`https://${domain}`, `http://${domain}`];

  for (const u of tryUrls) {
    try {
      await page.goto(u, { waitUntil: "domcontentloaded", timeout: ARGS.timeoutMs });
      await page.waitForTimeout(ARGS.settleMs);
      navigated = true;
      finalUrl = page.url();
      break;
    } catch {}
  }

  if (!navigated) {
    await context.close().catch(() => {});
    return {
      final_url: "",
      endpoints_all: new Set(),
      endpoints_tracking: new Set(),
      set_cookie_headers: new Set(),
      cookies: [],
      tracking_cookies: [],
      local_storage_keys: [],
      session_storage_keys: [],
      canvas_fp_called: false,
    };
  }

  // Pass interstitials if needed
  try {
    await passInterstitials(page, { notes: "" }, path.join(ARGS.evidence, safeSlug(domain), device), 0, 2);
  } catch {}

  // State action: S1 reject / S2 accept
  if (state === "S1") {
    try {
      if (await findClickableByText(page, REJECT_RE)) {
        await clickBestEffort(page, REJECT_RE);
      } else {
        await rejectViaPreferences(
          page,
          { notes: "" },
          path.join(ARGS.evidence, safeSlug(domain), device),
          0,
          () => {
            rec.summarizeTracking();
          }
        );
      }
    } catch {}
  } else if (state === "S2") {
    try { await clickBestEffort(page, ACCEPT_RE); } catch {}
  }

  await page.waitForTimeout(waitMs);

  const t = rec.summarizeTracking();
  const snap = await rec.snapshot(page, context, captureCanvasFp);

  finalUrl = page.url();

  await context.close().catch(() => {});

  return {
    final_url: finalUrl,
    endpoints_all: t.endpoints_all,
    endpoints_tracking: t.endpoints_tracking,
    set_cookie_headers: t.set_cookie_headers,
    cookies: snap.cookies,
    tracking_cookies: snap.tracking_cookies,
    local_storage_keys: snap.local_storage_keys,
    session_storage_keys: snap.session_storage_keys,
    canvas_fp_called: snap.canvas_fp_called,
  };
}

async function measureDP3Differential(
  browser: Browser,
  cand: CandidateRow,
  device: DeviceKind
): Promise<{
  dp3_flag_adv: number;
  pre_tracking_count: number;
  reject_tracking_count: number;
  accept_tracking_count: number;
  pre_only_endpoints_count: number;
  pre_only_cookies_count: number;
  pre_only_endpoints_sample: string;
  pre_only_cookies_sample: string;
  canvas_fp_pre: number;
}> {
  const domain = normalizeDomain(cand.domain);
  logStep(domain, device, 0, "DP3 differential: measuring S0 pre-consent");
  const S0 = await measureConsentState(browser, cand, device, "S0", ARGS.dp3WaitMs, ARGS.dp3CaptureCanvasFp);
  logStep(domain, device, 0, "DP3 differential: measuring S1 reject");
  const S1 = await measureConsentState(browser, cand, device, "S1", ARGS.dp3WaitMs, ARGS.dp3CaptureCanvasFp);
  logStep(domain, device, 0, "DP3 differential: measuring S2 accept");
  const S2 = ARGS.dp3IncludeAccept ? await measureConsentState(browser, cand, device, "S2", ARGS.dp3WaitMs, ARGS.dp3CaptureCanvasFp) : null;

  const preOnlyEndpoints = differentialSet(S0.endpoints_tracking, S1.endpoints_tracking);
  const preOnlyCookies = differentialCookies(S0.tracking_cookies, S1.tracking_cookies);

  const dp3 = (preOnlyEndpoints.length > 0 || preOnlyCookies.length > 0) ? 1 : 0;

  const cookieNames = preOnlyCookies.map((c: any) => String(c?.name || "")).slice(0, 5).join(";");
  const endpointSample = preOnlyEndpoints.slice(0, 5).join(";");

  return {
    dp3_flag_adv: dp3,
    pre_tracking_count: S0.endpoints_tracking.size,
    reject_tracking_count: S1.endpoints_tracking.size,
    accept_tracking_count: S2 ? S2.endpoints_tracking.size : 0,
    pre_only_endpoints_count: preOnlyEndpoints.length,
    pre_only_cookies_count: preOnlyCookies.length,
    pre_only_endpoints_sample: endpointSample,
    pre_only_cookies_sample: cookieNames,
    canvas_fp_pre: S0.canvas_fp_called ? 1 : 0,
  };
}

// ---------- Core runner ----------
async function runOne(browser: Browser, cand: CandidateRow, device: DeviceKind, runId: number): Promise<Obs> {
  const domain = normalizeDomain(cand.domain);
  const stratum = cand.stratum;

  const evidenceDir = path.join(ARGS.evidence, safeSlug(domain), device);
  ensureDir(evidenceDir);

  logStep(domain, device, runId, "Starting");

  const obs: Obs = {
    // IDs
    domain,
    stratum,
    source: cand.source ?? "",
    source_rank: cand.source_rank ?? "",
    run_id: runId,
    device,
    viewport: device === "mobile" ? "390x844" : "1366x768",
    locale: ARGS.locale,
    geo_country: "TH",
    datetime_utc: nowIsoUtc(),

    // nav results
    final_url: "",
    status: "ok",
    blocked_reason: "",

    // UI presence
    banner_present: 0,
    banner_type: "",
    cms_cmp_vendor: "unknown",
    language_detected: "",

    // buttons (first layer)
    accept_all_first_layer: 0,
    reject_all_first_layer: 0,
    manage_first_layer: 0,
    close_button_present: 0,

    // click depth
    clicks_accept_all: "",
    clicks_reject_all: "",
    delta_clicks: "",
    steps_reject_description: "",
    reject_success: 0,
    dp1_reason: "",
    dp2_reason: "",

    // toggles (best-effort)
    has_toggle_analytics: 0,
    has_toggle_ads: 0,
    default_on_analytics: 0,
    default_on_ads: 0,
    has_reject_all_anywhere: "",

    // network (third-party unique hosts)
    third_party_before_consent: 0,
    third_party_after_reject: "",
    tracking_after_reject_flag: "",
    tracking_requests_count: 0,
    tracking_cookies_count: 0,

    // Derived DP flags
    dp1_flag: "",
    dp2_flag: "",
    dp3_flag: "",
    dp4_flag: "",
    dp5_flag: "",
    dp6_flag: "",
    dp7_flag: "",
    dp8_flag: "",

    // DP3 differential columns (from measureDP3Differential)
    dp3_flag_adv: "",
    dp3_pre_tracking_count: "",
    dp3_reject_tracking_count: "",
    dp3_accept_tracking_count: "",
    dp3_pre_only_endpoints_count: "",
    dp3_pre_only_cookies_count: "",
    dp3_pre_only_endpoints_sample: "",
    dp3_pre_only_cookies_sample: "",
    canvas_fp_preconsent: "",

    // evidence paths
    screenshot_path_banner: "",
    screenshot_path_manage: "",
    notes: "",
  };

  // ---------- DP3 Differential measurement (if enabled) ----------
  if (ARGS.dp3) {
    logStep(domain, device, runId, "Starting DP3 differential");
    try {
      const dp3res = await measureDP3Differential(browser, cand, device);
      obs.dp3_flag_adv = dp3res.dp3_flag_adv;
      obs.dp3_pre_tracking_count = dp3res.pre_tracking_count;
      obs.dp3_reject_tracking_count = dp3res.reject_tracking_count;
      obs.dp3_accept_tracking_count = dp3res.accept_tracking_count;
      obs.dp3_pre_only_endpoints_count = dp3res.pre_only_endpoints_count;
      obs.dp3_pre_only_cookies_count = dp3res.pre_only_cookies_count;
      obs.dp3_pre_only_endpoints_sample = dp3res.pre_only_endpoints_sample;
      obs.dp3_pre_only_cookies_sample = dp3res.pre_only_cookies_sample;
      obs.canvas_fp_preconsent = dp3res.canvas_fp_pre;
    } catch (e: any) {
      obs.notes += `dp3_diff_failed: ${e.message}; `;
      logStep(domain, device, runId, "DP3 differential failed");
    }
  }

  // ---------- Main navigation and interaction ----------
  logStep(domain, device, runId, "Navigating");
  const contextOptions: any = {
    locale: ARGS.locale,
    ignoreHTTPSErrors: true,
    viewport: device === "mobile" ? { width: 390, height: 844 } : { width: 1366, height: 768 },
    userAgent:
      device === "mobile"
        ? "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Mobile/15E148 Safari/604.1"
        : undefined,
    isMobile: device === "mobile",
  };

  const context: BrowserContext = await browser.newContext(contextOptions);
  const page: Page = await context.newPage();

  const allReqUrlsBefore: string[] = [];

  const onRequest = (req: Request) => {
    try {
      const u = req.url();
      allReqUrlsBefore.push(u);
    } catch {}
  };

  page.on("request", onRequest);

  // Navigate with https -> fallback http if needed
  const tryUrls = cand.domain.startsWith("http")
    ? [cand.domain]
    : [`https://${domain}`, `http://${domain}`];

  let navigated = false;
  let lastErr = "";

  for (const u of tryUrls) {
    try {
      logStep(domain, device, runId, `Trying ${u}`);
      await page.goto(u, { waitUntil: "domcontentloaded", timeout: ARGS.timeoutMs });
      await page.waitForTimeout(ARGS.settleMs);
      await passInterstitials(page, obs, evidenceDir, runId, 3);

      obs.final_url = page.url();
      navigated = true;
      break;
    } catch (e: any) {
      lastErr = String(e?.message || e).slice(0, 200);
    }
  }

  if (!navigated) {
    obs.status = "error";
    obs.blocked_reason = lastErr || "navigation failed";
    await context.close();
    logStep(domain, device, runId, "Navigation failed");
    return obs;
  }

  // Check for blocking pages
  if (await isBlockedPage(page)) {
    obs.status = "blocked";
    obs.blocked_reason = "generic_blocked_page";
    logStep(domain, device, runId, "Blocked page detected");
    await context.close();
    return obs;
  }

  if (await detectTurnstile(page)) {
    logStep(domain, device, runId, "Cloudflare Turnstile detected");
    await Promise.race([
      page.waitForFunction(() => {
        const iframe = document.querySelector("iframe[src*='challenges.cloudflare.com']");
        return !iframe;
      }, { timeout: 15000 }),
      page.waitForTimeout(15000)
    ]).catch(() => {});
    obs.status = "blocked_by_antibot";
    obs.blocked_reason = "cloudflare_turnstile";
    logStep(domain, device, runId, "Turnstile timeout, marking blocked");
    await context.close();
    return obs;
  }

  logStep(domain, device, runId, "Collecting pre-consent requests");
  // Collect initial third-party requests after page load
  const afterUrls: string[] = [];
  page.on("request", (req) => afterUrls.push(req.url()));

  await page.waitForTimeout(2500); // allow some requests

  const siteHost = hostOf(page.url());
  const tp = new Set<string>();
  for (const u of afterUrls) {
    const h = hostOf(u);
    if (h && isThirdParty(h, siteHost)) tp.add(domainRoot(h));
  }
  obs.third_party_before_consent = tp.size;

  logStep(domain, device, runId, "Scanning banner and CMP");
  // Basic scans
  obs.language_detected = await detectLang(page);
  obs.cms_cmp_vendor = await detectCmpVendor(page);
  obs.banner_present = await bannerPresentHeuristic(page);

  // Evidence: banner screenshot
  const bannerShot = path.join(evidenceDir, `run${runId}_banner.png`);
  try {
    await page.screenshot({ path: bannerShot });
    obs.screenshot_path_banner = bannerShot;
  } catch (e: any) {
    obs.notes += `banner screenshot failed: ${String(e?.message || e).slice(0, 120)}; `;
  }

  // Detect buttons in first layer
  const acceptFound = await findClickableByText(page, ACCEPT_RE);
  const rejectFound = await findClickableByText(page, REJECT_RE);
  const manageFound = await findClickableByText(page, MANAGE_RE);
  const closeFound  = await findClickableByText(page, CLOSE_RE);

  obs.accept_all_first_layer = acceptFound ? 1 : 0;
  obs.reject_all_first_layer = rejectFound ? 1 : 0;
  obs.manage_first_layer     = manageFound ? 1 : 0;
  obs.close_button_present   = closeFound ? 1 : 0;

  // DP1 detection (Asymmetric Choice)
  if (obs.accept_all_first_layer === 1 && obs.reject_all_first_layer === 0) {
    obs.dp1_flag = 1;
    obs.dp1_reason = "no_reject_in_first_layer";
  } else if (obs.accept_all_first_layer === 1 && obs.manage_first_layer === 1 && obs.reject_all_first_layer === 0) {
    obs.dp1_flag = 1;
    obs.dp1_reason = "reject_in_manage";
  } else {
    obs.dp1_flag = 0;
  }

  // Click depth for accept
  if (obs.accept_all_first_layer) {
    obs.clicks_accept_all = 1;
  } else if (obs.manage_first_layer) {
    const a = await acceptViaPreferences(page, obs, evidenceDir, runId);
    if (a.ok) {
      obs.clicks_accept_all = a.clickCount;
      obs.notes += `accept_via_preferences:${a.details}; `;
    } else {
      obs.clicks_accept_all = 1; // fallback
    }
  } else {
    obs.clicks_accept_all = 0;
  }

  logStep(domain, device, runId, "Measuring reject path");
  // Click depth for reject
  if (obs.reject_all_first_layer) {
    obs.clicks_reject_all = 1;
    obs.reject_success = 1;
  } else if (obs.manage_first_layer) {
    const r = await rejectViaPreferences(page, obs, evidenceDir, runId);
    if (r.ok) {
      obs.clicks_reject_all = r.clickCount;
      obs.steps_reject_description = r.details;
      obs.reject_success = 1;
      obs.has_reject_all_anywhere = 1;
    } else {
      obs.clicks_reject_all = r.clickCount;
      obs.steps_reject_description = r.details;
      obs.reject_success = 0;
    }
  } else {
    obs.clicks_reject_all = 0;
    obs.reject_success = 0;
  }

  logStep(domain, device, runId, "Measuring DP1 and DP2");
  // DP2 detection (Obstruction)
  if (obs.clicks_accept_all !== "" && obs.clicks_reject_all !== "") {
    const delta = Number(obs.clicks_reject_all) - Number(obs.clicks_accept_all);
    obs.delta_clicks = delta;
    if (delta >= 2) {
      obs.dp2_flag = 1;
      obs.dp2_reason = "delta_clicks";
    } else if (obs.reject_success === 1 && obs.steps_reject_description?.includes('toggles')) {
      obs.dp2_flag = 1;
      obs.dp2_reason = "requires_toggling";
    } else {
      obs.dp2_flag = 0;
    }
  }

  logStep(domain, device, runId, "Inspecting manage/setup for DP3");
  // Try to open manage for DP3 measurement (if not already opened)
  let manageOpened = false;
  if (manageFound && !obs.reject_success) {
    try {
      await clickBestEffort(page, MANAGE_RE);
      await page.waitForTimeout(1500);
      manageOpened = true;
    } catch (e: any) {
      obs.notes += `manage open failed: ${String(e?.message || e).slice(0, 120)}; `;
    }
  } else if (manageFound) {
    // Already opened via reject flow, but we need to scan toggles anyway
    manageOpened = true;
  }

  if (manageOpened) {
    // Measure toggles for DP3
    const t = await scanTogglesBestEffort(page);
    obs.has_toggle_analytics = t.has_analytics;
    obs.has_toggle_ads = t.has_ads;
    obs.default_on_analytics = t.default_on_analytics;
    obs.default_on_ads = t.default_on_ads;
    obs.dp3_flag = (t.default_on_analytics || t.default_on_ads) ? 1 : 0;
  }

  logStep(domain, device, runId, "Measuring DP4 and DP6");
  // DP4: Visual Salience (measure before any clicks that might change UI)
  const salience = await measureButtonSalience(page);
  obs.dp4_flag = salience.dp4Flag;
  obs.notes += `areaRatio=${salience.areaRatio.toFixed(2)}, contrastDiff=${salience.contrastDiff.toFixed(0)}; `;

  // DP6: Vagueness (needs banner text)
  const bannerElem = page.locator('div[class*="cookie"], div[id*="cookie"], .cookie-banner').first();
  const bannerText = await bannerElem.innerText().catch(() => '');
  obs.dp6_flag = detectVagueness(bannerText);

  logStep(domain, device, runId, "Measuring DP5 and DP7");
  // Now attempt to actually reject for DP5/DP7
  let rejectClicked = false;

  const afterReqUrls: string[] = [];
  const afterListener = (req: Request) => {
    try { afterReqUrls.push(req.url()); } catch {}
  };

  page.off("request", onRequest);
  page.on("request", afterListener);

  if (rejectFound) {
    try {
      await clickBestEffort(page, REJECT_RE);
      rejectClicked = true;
    } catch {}
  } else if (manageOpened) {
    const innerReject = await findClickableByText(page, REJECT_RE);
    if (innerReject) {
      await clickBestEffort(page, REJECT_RE);
      rejectClicked = true;
    } else {
      obs.steps_reject_description += " | no reject-all found (manual coding needed)";
    }
  }

  if (rejectClicked) {
    obs.has_reject_all_anywhere = 1;
    try {
      await page.waitForTimeout(2500);

      // DP5: Cookie Wall
      obs.dp5_flag = await detectCookieWall(page);

      // DP7: Choice Not Respected
      const tracking = await measureTrackingAfterReject(page, context);
      obs.tracking_requests_count = tracking.trackingRequests;
      obs.tracking_cookies_count = tracking.trackingCookies;
      obs.tracking_after_reject_flag = tracking.trackingRequests > 0 ? 1 : 0;
      obs.dp7_flag = tracking.dp7Flag;
      obs.notes += `trackingReqs=${tracking.trackingRequests}, trackingCookies=${tracking.trackingCookies}; `;
    } catch (e: any) {
      obs.notes += `after-reject measure failed: ${String(e?.message || e).slice(0, 120)}; `;
    }
  } else {
    obs.has_reject_all_anywhere = obs.reject_all_first_layer || manageOpened ? "" : 0;
  }

  logStep(domain, device, runId, "Measuring DP8");
  // DP8: Withdrawal Difficulty
  obs.dp8_flag = await detectWithdrawalDifficulty(page);

  await context.close();
  logStep(domain, device, runId, "Finished");
  return obs;
}

// ---------- Main ----------
const OUTPUT_HEADER = [
  "domain","stratum","source","source_rank","run_id","device","viewport","locale","geo_country","datetime_utc",
  "final_url","status","blocked_reason",
  "banner_present","banner_type","cms_cmp_vendor","language_detected",
  "accept_all_first_layer","reject_all_first_layer","manage_first_layer","close_button_present",
  "clicks_accept_all","clicks_reject_all","delta_clicks","steps_reject_description","reject_success",
  "dp1_flag","dp1_reason",
  "dp2_flag","dp2_reason",
  "has_toggle_analytics","has_toggle_ads","default_on_analytics","default_on_ads","dp3_flag",
  "dp4_flag",
  "dp5_flag",
  "dp6_flag",
  "third_party_before_consent","third_party_after_reject","tracking_after_reject_flag","tracking_requests_count","tracking_cookies_count","dp7_flag",
  "dp8_flag",
  "dp3_flag_adv","dp3_pre_tracking_count","dp3_reject_tracking_count","dp3_accept_tracking_count","dp3_pre_only_endpoints_count","dp3_pre_only_cookies_count","dp3_pre_only_endpoints_sample","dp3_pre_only_cookies_sample","canvas_fp_preconsent",
  "screenshot_path_banner","screenshot_path_manage","notes"
];

function readCandidates(filePath: string): CandidateRow[] {
  const raw0 = fs.readFileSync(filePath, "utf-8");
  const raw = raw0.replace(/^\uFEFF/, "");

  const recs = parseCsv(raw, {
    columns: true,
    skip_empty_lines: true,
    trim: true,
    bom: true,
  }) as any[];

  const out = recs
    .map((r) => {
      const domRaw = r.domain ?? r["\ufeffdomain"];
      const domain = normalizeDomain(domRaw);
      return {
        ...r,
        domain,
        stratum: String(r.stratum ?? "").trim(),
        source: String(r.source ?? ""),
        source_rank: String(r.source_rank ?? ""),
      } as CandidateRow;
    })
    .filter((r) => r.domain.length > 0 && r.stratum.length > 0);

  const dropped = recs.length - out.length;
  if (dropped > 0) {
    console.warn(`[WARN] Dropped ${dropped} row(s) with empty domain/stratum`);
  }
  return out;
}

async function main() {
  ensureDir(ARGS.evidence);

  const candidates = readCandidates(ARGS.input);

  const uniqDomains = new Set(candidates.map((c) => c.domain));
  console.log(`Loaded ${candidates.length} rows, unique domains: ${uniqDomains.size}`);

  const doneKeys = ARGS.resume ? loadExistingKeys(ARGS.out) : new Set<string>();
  if (ARGS.resume) console.log(`Resume enabled. Already done keys: ${doneKeys.size}`);

  const tasks: { cand: CandidateRow; device: DeviceKind; runId: number }[] = [];
  for (const cand of candidates) {
    for (const device of ["desktop","mobile"] as DeviceKind[]) {
      for (let runId = 1; runId <= ARGS.runs; runId++) {
        const key = `${normalizeDomain(cand.domain)}|${device}|${runId}`;
        if (doneKeys.has(key)) continue;
        tasks.push({ cand, device, runId });
      }
    }
  }

  console.log(`Pending tasks: ${tasks.length} (concurrency=${ARGS.concurrency}, runs=${ARGS.runs}, headful=${ARGS.headful})`);

  let browser: Browser | undefined;
  let done = 0;
  let failed = 0;

  try {
    browser = await chromium.launch({ headless: !ARGS.headful });
    const limit = pLimit(ARGS.concurrency);

    const promises = tasks.map((t) =>
      limit(async () => {
        try {
          const obs = await runOne(browser!, t.cand, t.device, t.runId);
          writeCsvRowAppend(ARGS.out, OUTPUT_HEADER, obs);
          done++;
          if (done % 25 === 0) console.log(`Progress: ${done}/${tasks.length} appended.`);
        } catch (e: any) {
          failed++;
          console.error(`Task failed for ${t.cand.domain}|${t.device}|${t.runId}: ${e.message}`);
          const fallback: Obs = {
            domain: normalizeDomain(t.cand.domain),
            stratum: t.cand.stratum,
            source: t.cand.source ?? "",
            source_rank: t.cand.source_rank ?? "",
            run_id: t.runId,
            device: t.device,
            viewport: t.device === "mobile" ? "390x844" : "1366x768",
            locale: ARGS.locale,
            geo_country: "TH",
            datetime_utc: nowIsoUtc(),
            final_url: "",
            status: "script_error",
            blocked_reason: String(e?.message || e).slice(0, 200),
            banner_present: 0,
            banner_type: "",
            cms_cmp_vendor: "unknown",
            language_detected: "",
            accept_all_first_layer: 0,
            reject_all_first_layer: 0,
            manage_first_layer: 0,
            close_button_present: 0,
            clicks_accept_all: "",
            clicks_reject_all: "",
            delta_clicks: "",
            steps_reject_description: "",
            reject_success: 0,
            dp1_reason: "",
            dp2_reason: "",
            has_toggle_analytics: 0,
            has_toggle_ads: 0,
            default_on_analytics: 0,
            default_on_ads: 0,
            has_reject_all_anywhere: "",
            third_party_before_consent: 0,
            third_party_after_reject: "",
            tracking_after_reject_flag: "",
            tracking_requests_count: 0,
            tracking_cookies_count: 0,
            dp1_flag: "",
            dp2_flag: "",
            dp3_flag: "",
            dp4_flag: "",
            dp5_flag: "",
            dp6_flag: "",
            dp7_flag: "",
            dp8_flag: "",
            dp3_flag_adv: "",
            dp3_pre_tracking_count: "",
            dp3_reject_tracking_count: "",
            dp3_accept_tracking_count: "",
            dp3_pre_only_endpoints_count: "",
            dp3_pre_only_cookies_count: "",
            dp3_pre_only_endpoints_sample: "",
            dp3_pre_only_cookies_sample: "",
            canvas_fp_preconsent: "",
            screenshot_path_banner: "",
            screenshot_path_manage: "",
            notes: "Unhandled script error",
          };
          writeCsvRowAppend(ARGS.out, OUTPUT_HEADER, fallback);
        }
      })
    );

    await Promise.all(promises);
  } finally {
    if (browser) await browser.close();
  }

  console.log(`Done. appended=${done}, failed=${failed}`);
  console.log(`Output: ${ARGS.out}`);
  console.log(`Evidence dir: ${ARGS.evidence}`);
}

main().catch((e) => {
  console.error("Fatal:", e);
  process.exit(1);
});
